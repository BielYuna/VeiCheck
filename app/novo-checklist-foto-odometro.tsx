...
export default function NovoChecklistFotoOdometroScreen() {
  ...
  const hasPhoto = useMemo(() => !!photos[5], [photos]);
  const photoUri = useMemo(() => photos[5] || '', [photos]);
  ...
  const handleAvancar = () => {
    if (!hasPhoto) return;
    router.push('/novo-checklist-avarias-interior');
  };

  return (
    <ThemedView style={[styles.container, { paddingTop: insets.top }]}> 
      <View style={styles.progressBarContainer}>
        <View style={[styles.progressBar, { width: '83%', backgroundColor: '#51eb7c' }]} />
      </View>
      ...
      <Animated.View
        ...
        <View style={styles.photosContainer}>
          ...
          <ThemedText style={styles.photoHintText}>
            Toque para {hasPhoto ? 'remover' : 'adicionar'} a foto do odômetro
          </ThemedText>
        </View>
      </Animated.View>
