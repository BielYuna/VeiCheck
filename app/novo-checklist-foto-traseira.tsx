...
export default function NovoChecklistFotoTraseiraScreen() {
  ...
  const hasPhoto = useMemo(() => !!photos[1], [photos]);
  const photoUri = useMemo(() => photos[1] || '', [photos]);
  ...
  const handleAvancar = () => {
    if (!hasPhoto) return;
    router.push('/novo-checklist-avarias-traseira');
  };

  return (
    <ThemedView style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.progressBarContainer}>
        <View style={[styles.progressBar, { width: '90%', backgroundColor: '#51eb7c' }]} />
      </View>
      ...
      <Animated.View
        ...
        <View style={styles.photosContainer}>
          ...
          <ThemedText style={styles.photoHintText}>
            Toque para {hasPhoto ? 'remover' : 'adicionar'} a foto da traseira do veículo
          </ThemedText>
        </View>
      </Animated.View>
