...
export default function NovoChecklistAvariasLateralDireitaScreen() {
  ...
  const canSubmit = useMemo(() => {
    return lateralDireitaItems.every((item) => !!avarias[item]);
  }, [avarias]);
  ...
  const handleAvancar = () => {
    if (!canSubmit) return;
    // Navega para a foto da lateral esquerda do veículo
    router.push('/novo-checklist-foto-lateral-esquerda');
  };

  return (
    <ThemedView style={[styles.container, { paddingTop: insets.top }]}> 
      <View style={styles.progressBarContainer}>
        <View style={[styles.progressBar, { width: '56%', backgroundColor: '#51eb7c' }]} />
      </View>
      ...
      <Animated.View
        ...
        <ThemedText style={[styles.messageText, { fontSize: 24, fontWeight: '600', margin: 16 }]}> 
          Avarias lateral direita do veículo
        </ThemedText>
      </Animated.View>
      ...
      <Animated.View
        ...
        <View style={styles.avariasContainer}>
          <ThemedText style={[styles.sectionTitle, { marginBottom: 8 }]}>Parte lateral direita (marque uma opção):</ThemedText>
          <ScrollView
            style={styles.avariasTable}
            contentContainerStyle={styles.avariasTableContent}
            onLayout={(event) => setContainerHeight(event.nativeEvent.layout.height)}
            onContentSizeChange={(_, height) => setContentHeight(height)}>
            {lateralDireitaItems.map((item) => {
