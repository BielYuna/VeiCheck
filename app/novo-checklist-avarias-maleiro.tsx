...
export default function NovoChecklistAvariasMaleiroScreen() {
  ...
  const canSubmit = useMemo(() => {
    return maleiroItems.every((item) => !!avarias[item]);
  }, [avarias]);
  ...
  const handleAvancar = () => {
    if (!canSubmit) return;
    // Navega para a foto do odômetro
    router.push('/novo-checklist-foto-odometro');
  };
    ...
    <ThemedView style={[styles.container, { paddingTop: insets.top }]}> 
      ...
      <Animated.View
        ...
        <View style={styles.avariasContainer}>
          <ThemedText style={[styles.sectionTitle, { marginBottom: 8 }]}>Parte maleiro (marque uma opção):</ThemedText>
          <ScrollView
            style={styles.avariasTable}
            contentContainerStyle={styles.avariasTableContent}
            onLayout={(event) => setContainerHeight(event.nativeEvent.layout.height)}
            onContentSizeChange={(_, height) => setContentHeight(height)}>
            {maleiroItems.map((item) => {
